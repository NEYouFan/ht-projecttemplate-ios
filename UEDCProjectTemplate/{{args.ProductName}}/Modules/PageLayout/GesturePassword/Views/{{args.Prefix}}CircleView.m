...
@interface {{args.Prefix}}CircleView()
...
@implementation {{args.Prefix}}CircleView
...
- (void)setArrow:(BOOL)arrow
{
    ...
    // 遍历子控件，改变其是否有箭头
    [self.subviews enumerateObjectsUsingBlock:^({{args.Prefix}}Circle *circle, NSUInteger idx, BOOL *stop) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^({{args.Prefix}}Circle *circle, NSUInteger idx, BOOL *stop) {
...
{
    return [({{args.Prefix}}Circle *)[self.circleSet firstObject] state];
}

#pragma mark - 清空所有子控件的方向
- (void)resetAllCirclesDirect
{
    [self.subviews enumerateObjectsUsingBlock:^({{args.Prefix}}Circle *obj, NSUInteger idx, BOOL *stop) {
        [obj setAngle:0];
    }];
}
...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [{{args.Prefix}}CircleViewConst saveGesture:gesture Key:gestureOneSaveKey];
            ...
            BOOL equal = [gesture isEqual:[{{args.Prefix}}CircleViewConst getGestureWithKey:gestureOneSaveKey]]; // 匹配两次手势
...
{
    NSString *password = [{{args.Prefix}}CircleViewConst getGestureWithKey:gestureFinalSaveKey];
...
- (NSString *)getGestureResultFromCircleSet:(NSMutableArray *)circleSet
{
    ...
    for ({{args.Prefix}}Circle *circle in circleSet) {
        // 遍历取tag拼字符串
...
- (void)connectCirclesInRect:(CGRect)rect lineColor:(UIColor *)color
{
    ...
    for (int index = 0; index < self.circleSet.count; index++) {
        ...
        // 取出选中按钮
        {{args.Prefix}}Circle *circle = self.circleSet[index];
...
-(void)calAngleAndconnectTheJumpedCircle{
    ...
    //取出最后一个对象
    {{args.Prefix}}Circle *lastOne = [self.circleSet lastObject];
    
    //倒数第二个
    {{args.Prefix}}Circle *lastTwo = [self.circleSet objectAtIndex:(self.circleSet.count -2)];
    ...
    {{args.Prefix}}Circle *centerCircle = [self enumCircleSetToFindWhichSubviewContainTheCenterPoint:center];
 ...
 *  @return 点所在的圆
 */
- ({{args.Prefix}}Circle *)enumCircleSetToFindWhichSubviewContainTheCenterPoint:(CGPoint)point
{
    {{args.Prefix}}Circle *centerCircle;
    for ({{args.Prefix}}Circle *circle in self.subviews) {
        if (CGRectContainsPoint(circle.frame, point)) {
            centerCircle = circle;
        }
    }
